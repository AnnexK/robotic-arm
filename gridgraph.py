...
def metric_static(*args):
    # возвращает функцию, возвращающую константу
    return lambda *a : args[0]


class GridGraph:
    """Класс, моделирующий граф-решетку в трехмерном пространстве"""
    step = 0.1 # шаг сетки
    def __init__(self, start_point, end_point, metfoo=metric_static(step)):
        ...
        self.x_nedges = int(abs(start_point[0] - end_point[0]) * (1/GridGraph.step))
        self.y_nedges = int(abs(start_point[1] - end_point[1]) * (1/GridGraph.step))
        self.z_nedges = int(abs(start_point[2] - end_point[2]) * (1/GridGraph.step))
...
Возвращает None, если преобразование невозможно"""
        ...
        if in_x and in_y and in_z:
            return tuple(self.start[i] + GridGraph.step * point[i] for i in range(0,3))
        else:
            return None
...
в граф"""
        ...
        if in_x and in_y and in_z:
            try:
                ...
                # длина + кол-во собственного феромона
                edge = [self.metric(metric_data), 0.0]
                self.edges[edges_idx][point] = edge
                return self.edges[edges_idx][point]
        else:
            return (inf, 0.0) # ребра нет
